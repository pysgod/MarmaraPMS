  ...
  if (!patrol) {
    return (
      <div className="text-center py-16">
        <Shield size={64} className="text-dark-500 mx-auto mb-4" />
        <h2 className="text-xl font-semibold text-dark-200 mb-2">Devriye bulunamadı</h2>
        <button 
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-bold text-dark-50">{patrol.name}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-dark-800 rounded-2xl border border-dark-700 p-6">
            <h2 className="text-lg font-semibold text-dark-100 mb-4">Devriye Bilgileri</h2>
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-4">
                <div className="flex items-center gap-3 p-3 bg-dark-700/50 rounded-lg">
                  ...
                  <div>
                    <p className="text-xs text-dark-400">Atanan Personel</p>
                    <p className="text-dark-100">{patrol.assignee}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 p-3 bg-dark-700/50 rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="text-dark-100">{patrol.company}</p>
                  </div>
                </div>
              </div>
              <div className="space-y-4">
                <div className="flex items-center gap-3 p-3 bg-dark-700/50 rounded-lg">
                  ...
                  <div>
                    <p className="text-xs text-dark-400">Lokasyon</p>
                    <p className="text-dark-100">{patrol.location}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 p-3 bg-dark-700/50 rounded-lg">
                  ...
                  <div>
                    <p className="text-xs text-dark-400">Zaman Dilimi</p>
                    <p className="text-dark-100">{patrol.time}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="bg-dark-800 rounded-2xl border border-dark-700 p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-dark-100">Rota Bilgisi</h2>
            ...
            {/* Progress Bar */}
            <div className="relative h-2 bg-dark-700 rounded-full overflow-hidden mb-6">
              <div 
                ...
                <div key={point.id} className="flex items-center gap-4">
                  <div className="relative">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                      ...
                      point.status === 'active' ? 'bg-accent animate-pulse-glow' : 'bg-dark-600'
                    }`}>
                    ...
                    {index < routePoints.length - 1 && (
                      <div className={`absolute left-1/2 top-8 w-0.5 h-6 -translate-x-1/2 ${
                        point.status === 'completed' ? 'bg-green-500' : 'bg-dark-600'
                      }`} />
                    )}
                  </div>
                  <div className="flex-1 p-3 bg-dark-700/50 rounded-lg">
                    <div className="flex items-center justify-between">
                      <p className="font-medium text-dark-100">{point.name}</p>
                      <span className={`text-xs ${
                        point.status === 'completed' ? 'text-green-400' :
                        point.status === 'active' ? 'text-accent' : 'text-dark-400'
                      }`}>
                        {point.time}
                      </span>
                    </div>
        ...
        <div className="space-y-6">
          {/* Quick Actions */}
          <div className="bg-dark-800 rounded-2xl border border-dark-700 p-6">
            <h2 className="text-lg font-semibold text-dark-100 mb-4">Hızlı İşlemler</h2>
            <div className="space-y-3">
              ...
              <button className="w-full flex items-center gap-3 px-4 py-3 bg-dark-700/50 hover:bg-dark-700 text-dark-300 rounded-lg transition-colors">
                <Activity size={18} />
          ...
          <div className="bg-dark-800 rounded-2xl border border-dark-700 p-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-dark-400 text-sm">Toplam Nokta</span>
                <span className="text-dark-100 font-medium">5</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-dark-400 text-sm">Tamamlanan</span>
                <span className="text-green-400 font-medium">2</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-dark-400 text-sm">Bekleyen</span>
                <span className="text-amber-400 font-medium">3</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-dark-400 text-sm">Tahmini Süre</span>
                <span className="text-dark-100 font-medium">1 saat</span>
              </div>
            </div>
          </div>
          ...
          {/* Timeline */}
          <div className="bg-dark-800 rounded-2xl border border-dark-700 p-6">
            <h2 className="text-lg font-semibold text-dark-100 mb-4">Zaman Çizelgesi</h2>
            <div className="space-y-4">
              {[
                ...
                <div key={i} className="flex items-start gap-3">
                  <span className="text-xs text-dark-400 w-12">{item.time}</span>
                  <div className="w-2 h-2 rounded-full bg-accent mt-1.5" />
                  <span className="text-sm text-dark-200">{item.event}</span>
                </div>
              ))}
            </div>
          </div>

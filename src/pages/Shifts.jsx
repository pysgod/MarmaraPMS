  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-dark-100">Vardiya Yönetimi</h1>
          <p className="text-dark-400 mt-1">Firma bazlı vardiya tanımlamaları</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-1 bg-dark-800 rounded-xl border border-dark-700 overflow-hidden flex flex-col h-[calc(100vh-200px)]">
          <div className="p-4 border-b border-dark-700 bg-dark-700/50">
            <h3 className="font-semibold text-dark-200 mb-2">Firmalar</h3>
            ...
            {filteredCompanies.map(company => (
              ...
              >
                <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                   selectedCompanyId === company.id ? 'bg-accent' : 'bg-dark-700'
                }`}>
                  <Building2 size={16} className={selectedCompanyId === company.id ? 'text-white' : 'text-dark-400'} />
                </div>
          ...
          {selectedCompanyId ? (
            <div className="bg-dark-800 rounded-xl border border-dark-700 p-6">
              <div className="flex items-center gap-3 mb-6 pb-6 border-b border-dark-700">
                ...
                <div>
                  <h2 className="text-xl font-bold text-dark-100">{selectedCompany.name}</h2>
                  <p className="text-dark-400">Vardiya Tanımları</p>
                </div>
              </div>
              <CompanyShifts companyId={selectedCompanyId} />
            </div>
          ) : (
            <div className="bg-dark-800 rounded-xl border border-dark-700 p-12 text-center h-full flex flex-col items-center justify-center text-dark-400">
              <Building2 size={48} className="text-dark-600 mb-4" />
              <h3 className="text-lg font-medium text-dark-200 mb-2">Firma Seçin</h3>
